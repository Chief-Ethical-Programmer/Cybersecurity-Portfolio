...
- Create the VMs of your choice for the Internal/DMZ zone ref: [pfSense VM Setup Tutorial](https://www.youtube.com/watch?v=3cK8YHO0fWo) and install the pfsense from <https://www.pfsense.org/download/>
...
**1.4 Now verify services are running**
=======
...
**1.5 Access the Wazuh Dashboard** - 
=======
...
>>>>>>> aeafc5c (Updates Readme.md)
**4. Custom Decoders and Rules for Cowrie**

Cowrie's JSON log format is not natively understood by Wazuh's default ruleset without custom decoders, Cowrie events will be ingested but not meaningfully parsed or alerted on.
...
```
<<<<<<< HEAD
Since their is no open or default rules for cowrie I have created a basic detection rules. 
=======
...
<group name="cowrie,">
  ...
  <rule id="100102" level="10">
    ...
    <description>Cowrie: Successful login (honeypot credential accepted)</description>
  </rule>
...
```
<<<<<<< HEAD
For OpenCanary rules create a new group opencanary.
```xml 
=======
...
<group name="opencanary,">
  ...
  <rule id="100111" level="6">
    <decoded_as>opencanary-json</decoded_as>
    <field name="logtype">^XXXX$</field>
    <description>OpenCanary: HTTP request to canary service</description>
  </rule>

  <rule id="100112" level="6">
    <decoded_as>opencanary-json</decoded_as>
    <field name="logtype">^XXXX$</field>
    <description>OpenCanary: HTTPS/TLS interaction with canary service</description>
  </rule>
</group>
```
<<<<<<< HEAD
Now restart the manager to load the new rules
```bash
...
Finally the setup is over wazuh and below are the wazuh alerts:
![alt](../src/whimg4.png)
